...
>(({ className, ...props }, ref) => {
  ...
  return (
    <SliderPrimitive.Root
      ref={ref}
      className={cn(
        'relative flex w-full h-12 touch-none select-none items-center',
        className
      )}
      {...props}
    >
      <div className='absolute -top-[4px] left-0 text-[10px] text-primary tracking-wide'>{`${props.title}: ${props.value} ${props.name}`}</div>
